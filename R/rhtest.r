                  ...
                  treated_input_total, minimal_count_fdr)
{
  ...
  log.fc = log10((treated_ip/treated_input)/(untreated_ip/untreated_input))
  log.p = pmin(log.hyper.p, log.hypo.p)
  pvalues = exp(log.p)
  log.fdr = log10(p.adjust(pvalues, method = "fdr"))
  m = untreated_ip + untreated_input + treated_ip + treated_input
  ID = which(m > minimal_count_fdr)
  log.fdr_sig = log10(p.adjust(pvalues[ID], method = "fdr"))
